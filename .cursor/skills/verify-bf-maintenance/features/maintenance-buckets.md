...
- Bottom nav → Chores (`/maintenance`).
- From Systems, open Chores.
...
- **Open maintenance.** Run `node scripts/drive.mjs --feature maintenance-buckets --base-url <url>` (or click nav link `Chores`). Wait for page heading `getByRole('heading', { level: 2, name: 'Chores' })` — Layout chrome remains h1 `Maintenance`.
...
- Layout sticky header is always an h1 `Maintenance`; the page title is PageHeader h2 `Chores`. Use `level: 2` (or `main` landmark) so Playwright strict mode does not collide.
